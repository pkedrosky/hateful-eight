...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 920px) {
    ...
    #downloadBtn { grid-area: download; justify-self: end; }
...
function calculateImpactMetrics(frame) {
  ...
  return {
    ...
    otherSharePct,
    h8SegPct: h8SharePct,
    otherSegPct: otherSharePct,
    netTone: toneClass(totalPct),
    h8Tone: toneClass(h8Pts),
...
function renderImpact(frame) {
  ...
  impactBoxEl.innerHTML = `
    <div class="impact-summary">
      ...
      <div class="impact-summary-value ${metrics.netTone}">
        ${fmtSigned(metrics.totalPct, 1)}%
        <span class="impact-summary-sub">${fmtSigned(metrics.totalPts, 1)} pts</span>
      </div>
    </div>
    <div class="impact-bar-track">
      <div class="impact-bar-fill" style="width:100%">
        <div class="impact-segment h8" style="width:${metrics.h8SegPct.toFixed(1)}%"></div>
        <div class="impact-segment other" style="width:${metrics.otherSegPct.toFixed(1)}%"></div>
      </div>
    </div>
    ...
    <div class="impact-legend">
      <div class="impact-legend-item">
        ...
        <span class="impact-legend-meta ${metrics.h8Tone}">${metrics.h8SharePct.toFixed(1)}% share · ${fmtSigned(metrics.h8Pts, 1)} reconciled pts</span>
      </div>
      <div class="impact-legend-item">
        <span class="impact-swatch other"></span>
        <span class="impact-legend-name">Rest of S&P 500</span>
        <span class="impact-legend-meta ${metrics.otherTone}">${metrics.otherSharePct.toFixed(1)}% share · ${fmtSigned(metrics.otherPts, 1)} reconciled pts</span>
      </div>
    </div>
  `;
}
...
function currentFrame() {
  ...
  return frames[Math.max(0, Math.min(current, frames.length - 1))];
}

function exportFileName() {
  const frame = currentFrame();
  const frameEnd = frame ? frame.end : 'snapshot';
  return 'hateful-eight-' + currentWindow + '-' + frameEnd + '.png';
}
...
async function exportCurrentChartPng() {
  ...
  const infoH = 132;
  const footerH = 34;
  const chartY = headerH + infoH;
  const outW = W + (padX * 2);
  const outH = H + headerH + infoH + footerH;
  ...
  const winLabel = DATA.windowLabels[currentWindow] || currentWindow;
  ctx.fillStyle = '#0f172a';
  ctx.font = "700 24px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText(DATA.title, padX, 30);
  ctx.fillStyle = '#475569';
  ctx.font = "600 13px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText(winLabel + ' window ending ' + fmtDate(frame.end), padX, 50);
  ctx.font = "500 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText('Window: ' + fmtDate(frame.start) + ' to ' + fmtDate(frame.end), padX, 66);
  ...
  const boxX = padX;
  const boxY = headerH + 10;
  const boxW = W;
  const boxH = infoH - 16;
  drawRoundedRect(boxX, boxY, boxW, boxH, 10, '#fbfdff', '#d9e1ea');

  ctx.textAlign = 'left';
  ctx.fillStyle = '#64748b';
  ctx.font = "700 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText('CONTRIBUTION OVER PERIOD', boxX + 12, boxY + 20);
  ctx.font = "700 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText('Aggregate S&P Move', boxX + 12, boxY + 40);

  ctx.textAlign = 'right';
  ctx.fillStyle = toneColor(metrics.netTone);
  ctx.font = "700 28px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText(fmtSigned(metrics.totalPct, 1) + '%', boxX + boxW - 12, boxY + 42);
  ctx.fillStyle = '#64748b';
  ctx.font = "600 13px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText(fmtSigned(metrics.totalPts, 1) + ' pts', boxX + boxW - 12, boxY + 58);

  const barX = boxX + 12;
  const barY = boxY + 68;
  const barW = boxW - 24;
  const barH = 16;
  drawRoundedRect(barX, barY, barW, barH, 999, '#e8edf5', '#d9e1ea');

  const h8W = barW * (metrics.h8SegPct / 100);
  const otherW = Math.max(0, barW - h8W);
  drawRoundedRect(barX, barY, h8W, barH, 999, '#d4553b', null);
  drawRoundedRect(barX + h8W, barY, otherW, barH, 999, '#1f6fdb', null);

  const legendY = boxY + 102;
  ctx.textAlign = 'left';
  ctx.fillStyle = '#d4553b';
  ctx.fillRect(boxX + 12, legendY - 8, 10, 10);
  ctx.fillStyle = '#334155';
  ctx.font = "700 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText('Hateful Eight', boxX + 28, legendY);
  ctx.fillStyle = toneColor(metrics.h8Tone);
  ctx.font = "600 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText(metrics.h8SharePct.toFixed(1) + '% share · ' + fmtSigned(metrics.h8Pts, 1) + ' pts', boxX + 120, legendY);

  const rightLegendX = boxX + (boxW / 2) + 6;
  ctx.fillStyle = '#1f6fdb';
  ctx.fillRect(rightLegendX, legendY - 8, 10, 10);
  ctx.fillStyle = '#334155';
  ctx.font = "700 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText('Rest of S&P 500', rightLegendX + 16, legendY);
  ctx.fillStyle = toneColor(metrics.otherTone);
  ctx.font = "600 12px Geist, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
  ctx.fillText(metrics.otherSharePct.toFixed(1) + '% share · ' + fmtSigned(metrics.otherPts, 1) + ' pts', rightLegendX + 124, legendY);

  ctx.drawImage(img, padX, chartY, W, H);
...
async function handleDownloadPng() {
  ...
  try {
    await exportCurrentChartPng();
  } catch (err) {
    console.error(err);
    window.alert('PNG export failed. Try again once the chart is fully loaded.');
  } finally {
    exporting = false;
